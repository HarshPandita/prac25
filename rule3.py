...
class ConditionalRule(BaseRule):
    def __init__(self, condition_fn, rule):
        self.condition_fn = condition_fn  # (trip_id, expenses) => bool
        self.rule = rule

    def apply(self, grouped_expenses):
        results = []
        for trip_id, expenses in grouped_expenses.items():
            if self.condition_fn(trip_id, expenses):
                sub_result = self.rule.apply({trip_id: expenses})
...
class OrRule(BaseRule):
    ...
    def apply(self, grouped_expenses):
        ...
        for trip_id, all_errs in errors_by_trip.items():
            if all(all_errs):  # all rules failed
...
def total_greater_than(threshold):
    def fn(trip_id, expenses):
        return sum(e["amount"] for e in expenses) > threshold
    return fn
...
if __name__ == "__main__":
    ...
    engine = RuleEngine([
        AndRule(
            ...
            ConditionalRule(
                total_greater_than(400),
                ExpenseTypeRule({"food": 50})  # this rule enforced only if trip total > 400
            )
        )
    ])
